...
const RealStateTable = ({ data, isLoading, onStatusChange }) => {
    ...
    const columns = [
        ...
        {
            title: "properties", render: (value, row) => <TableProperties
                hasView={true}
...
const RowStatus = ({ row }) => {
    ...
    const { isLoading: specialLoading, mutate: specialRealStateMutate } = useMutation(
        (id) => getSpecialRealState(id),

        {
            onSuccess: async (data) => {
                if (data?.data?.status) {
                    await queryClient.invalidateQueries("realstates");
                } else {
                    Swal.fire({
                        icon: "error",
                        title: data?.response?.data?.message,
    ...
    return (
        ...
        <label className="inline-flex items-center me-5 cursor-pointer">
            {specialLoading ?
                <AiOutlineLoading3Quarters size={14} className="animate-spin text-maincolorgreen" />
                : <>
                    <input
                        type="checkbox"
                        value=""
                    ...
                    />
                    <div
                        className={`relative w-11 h-6 bg-gray-200 rounded-full dark:bg-gray-700 peer-checked:after:translate-x-[${row.special === 1 ? "-100%" : "0"
