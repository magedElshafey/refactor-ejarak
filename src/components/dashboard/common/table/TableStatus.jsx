...
const TableStatus = ({ status, onChange }) => {
    ...
    return (
        <div ref={ref} onClick={status === "pending" ? () => setActive(!active) : undefined} className="w-full relative -z-1">
            <div className={`${getStatusBackgroundColor(status)} py-2 w-full rounded-2xl flex items-center text-sm ${status === "pending" ? "justify-between px-4 cursor-pointer" : "justify-center"}`}>
                <p>{t(status)}</p>
                {
                    status === "pending" &&
                    <IoIosArrowDown />
