...
const Area = ({ min, max }) => {
  ...
  const [areaVal, setAreaVal] = useState(area || max); // إذا لم يتم تحديد area، يتم تعيين max

  useEffect(() => {
    setAreaVal(area || max); // تحديث القيمة عند تغير قيمة المساحة
  }, [area, max]);

  // لحساب النسبة المئوية لتغيير عرض شريط التمرير
    ...
    <div className="mb-4">
      ...
      <div className={style.container}>
        <input
          ...
          value={areaVal}
          onMouseUp={(e) => {
            dispatch(changeArea(Number(e.target.value))); // استخدام القيمة مباشرة لتحديث المساحة
          }}
          onChange={(event) => {
            setAreaVal(Number(event.target.value)); // تحديث حالة areaVal أثناء التغيير
          }}
